...
It can be seen that either a variable has 0 NA or more than 90% of examples have NA. So those variables are simply removed from the data.
...
In order to perform cross-validation, the training data is split into training (75%) and testing (25%), which is the method of random subsampling, using the function *createDataPartition*. This way it is possible to check the performance of the model on a data set other than the set it was trained on, obtaining an out-of-sample error.
...
The model used for prediction is a simple random forest method with all the default parameters. Random forests is one of the most widely used methods for predictions, making it the first choice for this project. Which we will see that has very good results and therefore is no need to fit other models.
...
Which correctly predicted all 20 cases.